...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (location) {
    return {
      ...
      description: `Book a professional wedding safa wala in ${location.name} for groom pagdi, family pagdi and baraati safa tying. On-location service from Wow Grooms.`,
      alternates: { canonical: `https://wowgrooms.in/${location.slug}` },
    };
  }
  ...
  if (page) {
    return {
      title: `${page.title}`,
      description: truncateDescription(`${page.intro} Check Wow Grooms availability for your wedding date.`),
      alternates: { canonical: `https://wowgrooms.in/${slug}` },
    };
  }
  return {};
}
...
export default async function AreaOrKeywordPage({ params }: Props) {
  ...
  if (location) {
    ...
    const breadcrumbSchema = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: "https://wowgrooms.in/" },
      { "@type": "ListItem", position: 2, name: "Service areas", item: "https://wowgrooms.in/#areas" },
      { "@type": "ListItem", position: 3, name: location.name, item: `https://wowgrooms.in/${location.slug}` },
    ] };
  ...
  const breadcrumbSchema = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
    { "@type": "ListItem", position: 1, name: "Home", item: "https://wowgrooms.in/" },
    { "@type": "ListItem", position: 2, name: page.title, item: `https://wowgrooms.in/${slug}` },
  ] };
