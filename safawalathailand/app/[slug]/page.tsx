...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (location) {
    return {
      ...
      description: `Book a professional wedding safa wala in ${location.name} for groom pagdi, family pagdi and baraati safa tying. On-location service from Safawala Thailand.`,
      alternates: { canonical: `https://safawalathailand.com/${location.slug}` },
    };
  }
  ...
  if (page) {
    return {
      title: `${page.title}`,
      description: truncateDescription(`${page.intro} Check Safawala Thailand availability for your wedding date.`),
      alternates: { canonical: `https://safawalathailand.com/${slug}` },
    };
  }
  return {};
}
...
export default async function AreaOrKeywordPage({ params }: Props) {
  ...
  if (location) {
    ...
    const breadcrumbSchema = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: "https://safawalathailand.com/" },
      { "@type": "ListItem", position: 2, name: "Service areas", item: "https://safawalathailand.com/#areas" },
      { "@type": "ListItem", position: 3, name: location.name, item: `https://safawalathailand.com/${location.slug}` },
    ] };
  ...
  const breadcrumbSchema = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
    { "@type": "ListItem", position: 1, name: "Home", item: "https://safawalathailand.com/" },
    { "@type": "ListItem", position: 2, name: page.title, item: `https://safawalathailand.com/${slug}` },
  ] };
