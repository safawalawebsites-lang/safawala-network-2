...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (location) {
    return {
      ...
      description: `Book a professional wedding safa wala in ${location.name} for groom pagdi, family pagdi and baraati safa tying. On-location service from Safawala Nashik.`,
      alternates: { canonical: `https://safawalanashik.com/${location.slug}` },
    };
  }
  ...
  if (page) {
    return {
      title: `${page.title}`,
      description: truncateDescription(`${page.intro} Check Safawala Nashik availability for your wedding date.`),
      alternates: { canonical: `https://safawalanashik.com/${slug}` },
    };
  }
  return {};
}
...
export default async function AreaOrKeywordPage({ params }: Props) {
  ...
  if (location) {
    ...
    const breadcrumbSchema = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: "https://safawalanashik.com/" },
      { "@type": "ListItem", position: 2, name: "Service areas", item: "https://safawalanashik.com/#areas" },
      { "@type": "ListItem", position: 3, name: location.name, item: `https://safawalanashik.com/${location.slug}` },
    ] };
  ...
  const breadcrumbSchema = { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
    { "@type": "ListItem", position: 1, name: "Home", item: "https://safawalanashik.com/" },
    { "@type": "ListItem", position: 2, name: page.title, item: `https://safawalanashik.com/${slug}` },
  ] };
