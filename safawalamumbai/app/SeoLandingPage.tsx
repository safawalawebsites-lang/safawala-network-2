...
function Brand({ footer = false }: { footer?: boolean }) {
  return <a className={`brand${footer ? " footer-brand" : ""}`} href="/" aria-label="Safawala Mumbai home"><span className="brand-logo-window"><img src="/safawala-logo-current.png" alt="Safawala" /></span><small>MUMBAI</small></a>;
}
...
export default function SeoLandingPage(props: Props) {
  ...
  const focusKeyword = location ? `wedding safa wala in ${location.name}` : `${service!.name.toLowerCase()} in Mumbai`;
  ...
  const secondCopy = location
    ...
    : `Choose a groom-only appointment, coordinated family styling or a full wedding-party package. We confirm the fabric, colour direction and number of artists before the Mumbai event.`;
      ...
      <main className="landing-main">
        <section className="landing-hero">
          <div className="landing-hero-inner section-shell">
            ...
            <InquiryForm pageName={pageName} areaPlaceholder={location ? `${location.name} venue or hotel` : "e.g. Andheri, Bandra"} />
          </div>
        </section>
        ...
        <section className="landing-faq section-shell" id="faq">
          <div className="section-heading"><div><p className="eyebrow">Unique page FAQs</p><h2>Questions about {pageName}.</h2></div><p>Answers are written for this page’s service or location context rather than copied from the Mumbai homepage.</p></div>
