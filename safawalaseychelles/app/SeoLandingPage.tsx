...
function Brand({ footer = false }: { footer?: boolean }) {
  return <a className={`brand${footer ? " footer-brand" : ""}`} href="/" aria-label="Safawala Seychelles home"><span className="brand-logo-window"><img src="/safawala-logo-current.png" alt="Safawala" /></span><small>SEYCHELLES</small></a>;
}
...
export default function SeoLandingPage(props: Props) {
  ...
  const focusKeyword = location ? `wedding safa wala in ${location.name}` : `${service!.name.toLowerCase()} in Seychelles`;
  ...
  const secondCopy = location
    ...
    : `Choose a groom-only appointment, coordinated family styling or a full wedding-party package. We confirm the fabric, colour direction and number of artists before the Seychelles event.`;
      ...
      <main className="landing-main">
        <section className="landing-hero">
          <div className="landing-hero-inner section-shell">
            ...
            <InquiryForm pageName={pageName} areaPlaceholder={location ? `${location.name} venue or hotel` : "e.g. Mont Fleuri, Bel Air"} />
          </div>
        </section>
        ...
        <section className="landing-about section-shell" id="about">
          <div className="landing-about-image"><img src={rotatedGallery[0][0]} alt={`${focusKeyword} – ${rotatedGallery[0][1]}`} width="1350" height="1688" /></div>
        ...
        <section className="landing-gallery" id="gallery">
          ...
          <div className="landing-gallery-grid">{rotatedGallery.slice(0, 6).map(([src, alt]) => <figure key={src}><img src={src} alt={`${alt} for ${pageName}`} loading="lazy" width="1350" height="1688" /></figure>)}</div>
        </section>

        <section className="landing-planning section-shell">
          <div><p className="eyebrow">Keyword-focused planning guide</p><h2>What to confirm before booking {focusKeyword}.</h2><p>{location ? `${location.summary} ${location.planning}` : `${service!.intro} ${service!.detail}`}</p><p>Share the event date, exact venue pin, reporting time, number of safas and outfit references. This allows the Safawala Seychelles team to suggest a realistic tying window, suitable wedding safa package and the correct number of artists.</p></div>
        ...
        <section className="landing-faq section-shell" id="faq">
          <div className="section-heading"><div><p className="eyebrow">Unique page FAQs</p><h2>Questions about {pageName}.</h2></div><p>Answers are written for this page’s service or location context rather than copied from the Seychelles homepage.</p></div>
