...
const faq = [
  ["How much does wedding safa tying cost?", "Pricing depends on the number of safas, selected fabric, accessories and time available for tying. Send your date, venue and approximate headcount for a tailored quote."],
  ["Do you provide safa tying at hotels and resorts?", "Yes. Our artists travel to hotels, resorts, homes and wedding venues across the city and nearby service area."],
  ...
  ["Do you travel for outstation weddings?", "Yes, subject to date and travel availability. Outstation bookings are planned as dedicated assignments with the venue pin and reporting time confirmed in advance."],
];
...
export default function HomeClient() {
  ...
  function submitInquiry(event: FormEvent<HTMLFormElement>) {
    ...
    const message = [
      "Hello Sharma Ji's Safawala, I would like to check wedding safa availability.",
      `Name: ${data.get("name")}`,
  ...
  return (
    <>
      <header className="site-header">
        <a className="brand" href="#top" aria-label="Sharma Ji's Safawala home">
          <span className="brand-logo-window"><img src="/safawala-logo-current.png" alt="Safawala" /></span><small>SHARMA JI'S</small>
        </a>
        <nav aria-label="Main navigation">
          <a href="#services">Services</a><a href="#gallery">Gallery</a><a href="#faq">FAQs</a>
        </nav>
      </header>
      ...
      <main id="top">
        <section className="hero section-shell">
          <div className="hero-copy">
            <p className="eyebrow">A Trusted Family Safa Business</p>
            <h1>Sharma Ji's Safas. Tied With Trust.</h1>
            <p className="hero-lede">Three generations of safa tying experience, brought to your wedding — warm, dependable groom pagdi and baraati safa service for every family.</p>
            ...
            <div className="hero-photo">
              ...
              <div className="photo-note"><span>Serving Sharma Ji's Safawala & nearby destinations</span><strong>31 unique locations</strong></div>
            </div>
          </div>
        ...
        <section className="seo-intro section-shell" aria-labelledby="rishikesh-safa-heading">
          <div className="seo-intro-heading">
            <p className="eyebrow">Family-run wedding safa service</p>
            <h2 id="rishikesh-safa-heading">A family safa business, trusted across generations.</h2>
          </div>
          <div className="seo-intro-copy"><p>Sharma Ji's Safawala provides <strong>wedding safa rental, groom pagdi styling and professional turban tying</strong>, carried forward through three generations of the same family business. Every safa is tied with the same care and attention shown to families for decades.</p><p>Couples and families can coordinate the fabric, colour, style, quantity and tying team in one enquiry — covering the groom, fathers, brothers, close family, groomsmen and the full wedding party.</p></div>
        ...
        <section className="editorial section-shell">
          ...
          <div className="editorial-copy"><p className="eyebrow">Family-business planning</p><h2>Calm preparation before the celebration begins.</h2><p>A trusted family service still needs a clear plan, so a safa booking only works well when the venue, guest count and preparation room are confirmed well ahead of the day.</p><p>We confirm the venue, reporting time and preparation room before the event. The groom receives an unhurried styling slot; family and baraatis follow in planned batches.</p><ul><li>On-location hotel, resort and home service</li><li>Groom, family and baraati styling plans</li><li>Colour coordination from outfit references</li><li>Travel planning for nearby destinations</li></ul></div>
        </section>
        ...
        <section className="explore-section section-shell" id="areas" aria-label="Explore wedding safa service pages">
          ...
          <article className={`explore-card explore-card-compact ${expandedAreas ? "is-expanded" : ""}`}>
            <div className="explore-card-head"><div><h2>Areas we serve</h2><p>{allLocations.length} areas across Vadodara.</p></div><button type="button" onClick={() => setExpandedAreas((value) => !value)} aria-expanded={expandedAreas}>{expandedAreas ? "Hide cities" : "View all"}</button></div>
            {expandedAreas && <div className="explore-chip-links">{allLocations.map((area) => <a href={`/${area.slug}`} key={area.slug}>{area.name}</a>)}</div>}
          </article>
          )}
