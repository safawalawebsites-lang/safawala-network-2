...
function Brand({ footer = false }: { footer?: boolean }) {
  return <a className={`brand${footer ? " footer-brand" : ""}`} href="/" aria-label="Sharma Ji's Safawala home"><span className="brand-logo-window"><img src="/safawala-logo-current.png" alt="Safawala" /></span><small>SHARMA JI'S</small></a>;
}
...
export default function SeoLandingPage(props: Props) {
  ...
  const focusKeyword = location ? `wedding safa wala in ${location.name}` : `${service!.name.toLowerCase()} in Sharma Ji's Safawala`;
  ...
  const secondCopy = location
    ...
    : `Choose a groom-only appointment, coordinated family styling or a full wedding-party package. We confirm the fabric, colour direction and number of artists before the Sharma Ji's Safawala event.`;
    ...
    <>
      ...
      <main className="landing-main">
        ...
        <section className="landing-faq section-shell" id="faq">
          <div className="section-heading"><div><p className="eyebrow">Unique page FAQs</p><h2>Questions about {pageName}.</h2></div><p>Answers are written for this page’s service or location context rather than copied from the Sharma Ji's Safawala homepage.</p></div>
